...
export default function Driver() {
  ...
  const [alert, setAlert] = useState<AlertProps>({ type: 1, message: "Route created successfully!" });
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAlert({
          ...
          message:
            result.code === 403
              ? "Routes can only be created after verification of account!"
              : "Failed to create route! Check for unfinished routes!",
        });
      }
    } catch (e) {
  ...
  const closeStops = () => setStops(false);
  const openStops = () => setStops(true);

  const get = (info: string) => sessionStorage.getItem(info) ?? "";
