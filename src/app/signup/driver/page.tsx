...
export default function Home() {
  ...
  const [alert, setAlert] = useState<AlertProps>({
    type: 1,
    message: "Please wait for verification mail before attempting to login!",
  });
  const [hideAlert, setHideAlert] = useState<boolean>(true);

  // Helper function for showing alerts
  const showAlert = (type: 1 | 2, message: string, duration = 3500) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!PHONE_NUMBER_PATTERN.test(phoneNumber)) {
        showAlert(2, "Invalid Phone Number! Must match format 0xxxxxxxxxx e.g. 07012934999", 4500);
        return;
      }

      if (password !== confirmPassword) {
        showAlert(2, "Ensure password and confirm password are the same!");
        return;
      }
      ...
      const response = await fetch(`${endpoint[0]}driver/signup/`, requestOptions);
      console.log(response.status);
      ...
      if (response.ok) {
        ...
        const result = await response.json();
        if (result.code === 400) {
          showAlert(2, "Password must be at least 8 characters long, contain an uppercase letter, a number, and a special character!");
        }
      } else {
        const text = JSON.parse(await response.text());
        showAlert(2, text.message);
      }

    } catch (err) {
      console.error("Signup error:", err);
      showAlert(2, "An unexpected error occurred. Please try again later.");
    } finally {
      setLoading(false);
      if (isOk) {
        setTimeout(() => route.push("/"), 4500);
